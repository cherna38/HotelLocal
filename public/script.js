...
function hotelLoop(hotelList, park, wifi, rate) {
    ...
    $(searchOutput).appendTo("#resultsTable tbody");
    //document.write('<table id ="resultsTable" border="3" cellspacing="4" cellpadding="4"><thead> <tr> <th>Hotel</th> <th>Parking</th> <th>Rating</th> <th>Wifi</th> </tr></thead> <tbody>' + searchOutput + '</tbody><table>');
}

//For a given hotel document, print it's attributes
function hotelPrint(hotel, park, wifi, rate) {

    var table = ''; // variable to make table with hotel info
    ...
    searchOutput += "<tr><td>" + hotel.id + "</td>";

    //Based on data in Firestore, print a readable output
    if (currentHotel.Parking == true) {
        searchOutput += "<td>free</td>";
    } else {
        searchOutput += "<td>none</td>";
    }

    searchOutput += "<td>" + currentHotel.Rating + "</td>";

    if (currentHotel.Wifi == true) {
        searchOutput += "<td>available</td> </tr>";

    } else {
        searchOutput += "<td>unavailable</td> </tr>";

    }

    //finalT = "<table id="resultsTable" style="width: 100 %" border="3" cellspacing="4" cellpadding="4"><thead> <tr> <th>Hotel</th> <th>Parking</th> <th>Rating</th> <th>Wifi</th> </tr></thead> <tbody>' + searchOutput + '</tbody><table>";
    //console.log(searchOutput);
    //$(searchOutput).appendTo("#resultsTable tbody");

    //setTimeout(finalT, 100);

    //document.write('<table id ="resultsTable" style="width:100%" border="3" cellspacing="4" cellpadding="4"><thead> <tr> <th>Hotel</th> <th>Parking</th> <th>Rating</th> <th>Wifi</th> </tr></thead> <tbody>' + searchOutput + '</tbody><table>');
}
...
searchButton.addEventListener("click", function () {
    ...
    if (inputCity == "Palo Alto") {
        ...
        searchOutput = 'Information for hotels in ' + inputCity + ' unavailable\n';
        window.alert("Sorry, we don't have info for hotels in that city yet.");
    }
    //document.write('<table id ="resultsTable" style="width:100%" border="3" cellspacing="4" cellpadding="4"><thead> <tr> <th>Hotel</th> <th>Parking</th> <th>Rating</th> <th>Wifi</th> </tr></thead> <tbody>' + searchOutput + '</tbody><table>');
    //document.write(finalT);
